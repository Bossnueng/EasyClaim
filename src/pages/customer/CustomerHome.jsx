...
const CustomerHome = () => {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 bg-gray-100 min-h-screen flex flex-col gap-6" style={{ boxSizing: "border-box", width: "100%" }}>
      <div className="w-full flex flex-col gap-6" style={{ boxSizing: "border-box" }}>
        
        {/* ==================== Header ==================== */}
        <header className="flex flex-col gap-1">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 m-0">สวัสดี, คุณ {user?.name || "-"}</h1>
          <p className="text-gray-500 text-sm m-0">ติดตามสถานะหรือแจ้งเคลมสินค้าใหม่ได้ง่ายๆ ที่นี่</p>
        </header>
        

        {/* ==================== Banner แจ้งเคลม ==================== */}
        <div 
          style={{boxSizing: "border-box",padding: "24px 28px",overflow: "hidden",}}
        ...
        >
          <div className="flex flex-col gap-1 text-center sm:text-left">
            <h3 className="text-lg sm:text-xl font-bold text-white m-0">แจ้งเคลมสินค้าใหม่ได้ทันที</h3>
            <p className="text-sm opacity-90 text-emerald-50 m-0">ใช้เวลาเพียงไม่กี่นาที เพื่อเริ่มต้นกระบวนการเคลมสินค้าของคุณ</p>
          </div>
          {/* ปุ่มแจ้งเคลมใหม่ */}
          <Button
            type="default"
        ...
        <section className="flex flex-col gap-4">
          <div className="flex justify-between items-center">
            <h2 className="text-lg sm:text-xl font-bold text-slate-800 m-0">รายการเคลมล่าสุด</h2>
            <Button
              type="link"
              ...
              {latestClaims.map((claim) => (
                ...
                >
                  <div className="flex flex-col gap-1 min-w-0">
                    <h4 className="font-bold text-base text-slate-800 m-0 truncate">{claim.productName}</h4>
                    <p className="text-xs text-gray-400 font-mono m-0 truncate">ID: {claim.claimId}</p>
                  </div>

                  <div className="flex flex-col items-end gap-1.5 shrink-0">{getStatusTag(claim.status)}
