...
const StaffChat = () => {
  ...
  // ดึง chatHistory จาก localStorage
  ...
  const handleSelectClaim = (claimId) => {
    ...
    setUnreadCounts((prev) => ({ ...prev, [claimId]: 0 }));
    ...
    setClaimsList((prev) =>
      prev.map((claim) =>
        claim.claimId === activeClaimId
          ? { ...claim, lastMessage: inputText, lastTime: currentTime }
          : claim
      )
    );
  ...
  return (
    <div
      style={{
        display: "flex",
        width: "100%",
        height: isMobile ? "100dvh" : "calc(100vh - 112px)",
    ...
    >
      ...
      >
        <div style={{ padding: isMobile ? "12px" : "16px", borderBottom: "1px solid #e2e8f0", display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {hasNoClaims ? (
            <div style={{ padding: "32px 16px", textAlign: "center" }}>
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<span style={{ color: "#94a3b8", fontSize: "13px" }}>ยังไม่มีรายการสนทนาใหม่เข้าระบบ</span>} />
            </div>
          ) : (
            <List
      ...
      {/* CHAT MAIN AREA */}
      <div style={{ flex: 1, display: isMobile && !showMobileChat ? "none" : "flex", flexDirection: "column", backgroundColor: "#ffffff", height: "100%", width: "100%" }}>
        {hasNoClaims ? (
            ...
            <Empty
              ...
              description={
                <div style={{ textAlign: "center" }}>
                  ...
                  <p style={{ margin: 0, color: "#94a3b8", fontSize: "13px" }}>เมื่อมีการยื่นคำร้องเคลมสินค้า รายการสนทนาจะปรากฏในช่องนี้</p>
                </div>
              }
            />
          </div>
          ...
          <>
            {/* Header */}
            <div style={{ padding: isMobile ? "10px 12px" : "12px 24px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between", backgroundColor: "#ffffff", gap: "8px" }}>
              {activeClaimInfo ? (
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ overflow: "hidden", textOverflow: "ellipsis" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "nowrap" }}>
                        <h3 style={{ margin: 0, fontSize: isMobile ? "14px" : "16px", fontWeight: "700", color: "#1e293b", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                          {activeClaimInfo.customerName}
                        </h3>
                        <Tag color="blue" style={{ fontSize: "10px", marginRight: 0, flexShrink: 0 }}>
                  ...
                  <Space size={isMobile ? 2 : 8}>
                    {!isMobile && <Button size="small" icon={<FileTextOutlined />}>ดูรายละเอียดเคส</Button>}
                    <Button
                      size="small"
                      type="primary"
                      icon={<CheckCircleOutlined />}
                      danger={activeClaimInfo.status === "เสร็จสิ้น"}
                      style={{ fontSize: isMobile ? "11px" : "12px", padding: isMobile ? "0 6px" : undefined }}
                    >
            ...
            {/* Messages Body */}
            <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? "10px" : "20px", backgroundColor: "#f8fafc", display: "flex", flexDirection: "column", gap: "12px" }}>
              {currentMessages.length === 0 ? (
                ...
                currentMessages.map((msg, index) => {
                  ...
                  const isPrevSameSender = index > 0 && currentMessages[index - 1].senderRole === msg.senderRole;

                  return (
                    <div key={msg.id} style={{ display: "flex", width: "100%", justifyContent: isMe ? "flex-end" : "flex-start" }}>
                      <div style={{ display: "flex", gap: isMobile ? "6px" : "12px", maxWidth: isMobile ? "90%" : "65%", marginLeft: isMe ? "auto" : "0", flexDirection: isMe ? "row-reverse" : "row" }}>
                        {!isMe && (
                          <div style={{ flexShrink: 0 }}>
                            {!isPrevSameSender ? (
                              <Avatar size={isMobile ? 28 : 36} icon={<UserOutlined />} style={{ backgroundColor: "#94a3b8", color: "#ffffff" }} />
                            ) : (
                              <div style={{ width: isMobile ? "28px" : "36px" }} />
                            )}
                          </div>
                        )}
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start" }}>
                          {!isPrevSameSender && (
                            <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "2px" }}>
                              <span style={{ fontSize: "11px", fontWeight: "600", color: "#475569" }}>{msg.senderName}</span>
                          ...
                          {isMe && (
                            <span style={{ fontSize: "9px", color: "#94a3b8", marginTop: "2px", display: "flex", alignItems: "center", gap: "2px" }}>
            ...
            {/* Input Area */}
            <div style={{ padding: isMobile ? "8px 10px" : "16px", backgroundColor: "#ffffff", borderTop: "1px solid #e2e8f0" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "6px", backgroundColor: "#f1f5f9", padding: "4px 8px", borderRadius: "9999px", border: "1px solid #cbd5e1" }}>
                <button type="button" style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", padding: "4px" }}>
                  <PaperClipOutlined style={{ fontSize: "16px" }} />
                </button>

                <input
                  type="text"
                  style={{ flex: 1, background: "transparent", border: "none", outline: "none", padding: "4px", fontSize: "13px", color: "#0f172a", minWidth: 0 }}
                  placeholder={activeClaimId ? `ตอบกลับ ${activeClaimId}...` : "เลือกเคสเพื่อพิมพ์..."}
                  value={inputText}
